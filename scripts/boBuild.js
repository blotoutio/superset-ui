...
function getPackages(packagePattern, tsOnly = false) {
  ...
  if (pattern === '*' && !tsOnly) {
    return `@blotoutio/!(${[...META_PACKAGES].join('|')})`;
  }
  if (!pattern.includes('*')) {
    pattern = `*${pattern}`;
  }
  const packages = [
    ...new Set(
      fastGlob
        .sync([
          `./node_modules/@blotoutio/${pattern}/src/**/*.${
  ...
  return `@blotoutio/${packages.length > 1 ? `{${packages.join(',')}}` : packages[0]}`;
}
...
if (shouldCleanup) {
  ...
  const dirtyModules = 'node_modules/@types/react,node_modules/@blotoutio';
