...
export default function transformProps(chartProps: ChartProps): BarRichTextProps {
  ...
  const { groupby, columns, metrics, boldText, headerFontSize, headerText } = formData;
  
  const groupkeys = data.map(datum => extractGroupbyLabel({ datum, groupby })).filter(onlyUnique);
  const colkeys = data.map(datum => extractColLabel({ datum, columns })).filter(onlyUnique);
  const transformedData = getSeriesData({data, colkeys, columns, metrics});
